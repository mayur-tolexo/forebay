...
| --- | --- | --- |
| A result on one hardware profile does not transfer to another | Measured, and more sharply than reasoned. Two nodes of one cluster, both 10 GbE, reading the same object from the same store, disagree about which side of the crossover they are on: the backend wins on one and the tier wins on the other by twenty five times | Results are quoted beyond their validity and the project is wrong in a way that looks measured, which is what quoting the second node alone would have done |
...
| --- | --- | --- |
| ~~Where the locality crossover sits between node-local bandwidth and a node's achievable share of backend fan-out~~ | 2 | **Found, and it sits between two nodes of the same cluster.** On a node whose local device reads at 314 MB/s and which reaches the store at 1043 MiB/s, the backend wins from four readers up: 190, 663 and 1043 MiB/s against a tier flat at 386, 431 and 394. On a node whose local device is NVMe and which reaches the same store at 110 MiB/s, the tier wins: 377, 1715 and 2767 against 74, 111 and 111. Both nodes have 10 GbE and read the same object, and the second run on each reproduced the first within five per cent. So the answer is a ratio rather than a number, between what a node's device gives and what that node can pull from the store, and both ends of it varied by an order of magnitude inside one cluster. **Kill criterion 2 fires on the first node and not on the second**, which is the shape of the risk rather than its resolution: Forebay targets the second kind and cannot assume it has one.<br><br>The margin on the winning node is inflated by its own weakness. It reaches nine per cent of its line rate to the store where the other node reaches eighty seven, from the same store on the same day, so part of what the tier beat there is a network path rather than a fast disk. What the tier would beat on a node with the first node's network and the second's device is unmeasured, and is between two point seven times and twenty five.<br><br>Earlier caveats stand: the tier figure has the page cache evicted and the eviction counted, and the store's first-touch reads vary between 34 and 110 MiB/s across runs on the slower path. |
...
| When freed capacity becomes observably available to a competing writer, which is what compute waits for, rather than when `unlink` returns | 1 | **Half answered.** In the filesystem's own accounting, immediately: releasing 8 GiB as eight extents on an idle ext4 filesystem, the space was there on the first reading taken after `unlink` returned, five times out of five, with the `unlink` itself taking about a millisecond. That is the half the agent rests on, since a headroom sized from a duration corrects the observed rate by what it gave back. The other half is unmeasured and needs a filesystem near full: a writer waits to allocate, and space present in the accounting is not the same as space it can have. Under concurrent writing this method answers neither, because free space then moves by the release and the writer together and cannot be attributed to one: a run under four writers saw 7.99 GiB of the 8 arrive within a millisecond and still refused the number, which is the right refusal and not an answer |
