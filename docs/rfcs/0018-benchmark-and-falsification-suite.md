...
| Whether staging a checkpoint locally beats writing it straight through, on a given node | Whether the checkpoint path is worth having at all, since it depends on the ratio between a node's device and its achievable share of the store, and that varied by more than an order of magnitude across the three machines measured | RFC-0013 |
| The service time above which a node agent reports itself unready, since a slow agent is worse than a stopped one: a stopped one fails its liveness probe and is replaced, while a slow one keeps taking reads that nothing rescues | Whether readiness protects a workload or merely describes the node | RFC-0017 |
