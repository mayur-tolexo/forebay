...
| Readiness computed from latency | **Built.** Every read is timed around what a caller waited for, and the same number goes to the histogram an operator reads and to the readiness an orchestrator probes at `/ready` |
| Serving the fast tier | **Built.** The agent answers ranged reads over a socket from the tier, missing to the durable backend. What is missing above it is a metadata server, so nothing mounts. Owned by [RFC-0007](0007-fast-tier-data-path.md) |
| Any interface to a control plane | **Built.** A control plane proposes a lease over HTTP and the node answers with what it decided; the agent also reports its capacity and what its tier holds. Guarded by a token, and not served without one |
